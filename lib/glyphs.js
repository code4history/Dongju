...
const DEFAULT_FONTSTACK = ["Open Sans Regular", "Arial Unicode MS Regular"]
...
function getFontStacks (style) {
  var fontStacks = {}
  fontStacks[DEFAULT_FONTSTACK.join(',')] = true
  traverse(style).forEach(function (x) {
    if (this.key === 'text-font') {
      if (Array.isArray(x)) {
        if (x[0] === 'step' && x[2] && x[2][0] === 'literal') {
          if (Array.isArray(x[2][1])) {
            fontStacks[x[2][1].join(',')] = true
          } else {
            fontStacks[x[2][1]] = true
          }
        } else if (x[0] === 'literal') {
          if (Array.isArray(x[1])) {
            fontStacks[x[1].join(',')] = true
          } else {
            fontStacks[x[1]] = true
          }
        } else {
